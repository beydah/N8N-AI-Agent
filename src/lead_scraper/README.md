...
1. **Flexible Triggering:** Powered by an interactive n8n Form where you can specify Location and Keyword.
2. **Multi-page Scrape:** Pages through up to 3 pages of Maps search results with built-in rate-limiting delays.
3. **Smart Deduplication:** Cross-checks scraped business profiles against the spreadsheet database beforehand.
4. **Data Enrichment:** Enriches leads with coordinates, website links, and formatted phone numbers.
5. **Quality Filter:** Skips leads lacking valid contact phone numbers.

---

## 🗺️ Process Layout

The following flowchart describes the operations inside the lead scraping pipeline:

![Lead Scraper Flow](./lead_scraper_flow.svg)
...
| Issue | Resolution |
| :--- | :--- |
| **No items are scraped** | Verify your Google Cloud project has billing enabled and the Places API is active. |
| **Google Sheets writing fails** | Verify that your column headers match the exact casing and names: `ID`, `Location`, `Keyword`, `Company`, `Phone`, `Link`, `Date`, `Status`. |
| **Duplicate entries appear** | Make sure the `ID` column in your spreadsheet is populated with the unique Google Maps ID (`place_id`). |
| **API rate limiting errors** | If Google returns errors, consider increasing the duration in the `Wait` nodes. |
